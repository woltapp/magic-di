...
def create_celery(
    ...
    with tempfile.NamedTemporaryFile(delete=True) as temp_file:
        yield Celery(
            ...
            backend=f"db+sqlite:///{temp_file.name}"
            if use_broker_and_backend
            else None,
        )
...
def test_async_function_based_tasks(
    run_celery: Celery, service_ping_task: InjectableCeleryTask
):
...
async def test_sync_function_based_tasks(
    run_celery: Celery, service_ping_task_sync: InjectableCeleryTask
):
...
def test_async_class_based_tasks(
    run_celery: Celery, service_ping_class_based_task: InjectableCeleryTask
):
...
def test_sync_class_based_tasks(
    run_celery: Celery, service_ping_class_based_task_sync: InjectableCeleryTask
):
...
async def test_async_function_based_tasks_inside_event_loop(
    ...
    with create_celery(injector, use_broker_and_backend=use_broker_and_backend) as app:
        ...
        @app.task
        async def ping_task(
            arg1: int, arg2: str, service: Service = Provide()
        ) -> tuple:
            mock()
            return arg1, arg2, service.is_alive()
